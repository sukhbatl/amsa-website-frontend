...
import { FaLinkedin, FaFacebook, FaInstagram, FaBuilding, FaBook, FaCalendar, FaGraduationCap, FaEnvelope, FaSchool } from "react-icons/fa";
...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      // If not logged in, redirect to public profile
  ...
  const loadProfile = async () => {
    try {
      setLoading(true);
      const data = await authFetch(`/api/user/profile/${id}`);
      setProfile(data);
      setError(null);
    } catch (err) {
              ...
              <div className="p-6 pt-4 space-y-4">
                {profile.email && (
                  <div className="flex items-start">
                    <FaEnvelope className="mr-3 mt-1 text-blue-600" />
                    <div>
                ...
                {profile.schoolYear && (
                  <div className="flex items-start">
                    <FaSchool className="mr-3 mt-1 text-indigo-600" />
                    <div>
                      <p className="text-xs text-gray-500 uppercase font-semibold">School Year</p>
                      <p className="text-gray-900 font-medium">{profile.schoolYear}</p>
                    </div>
                  </div>
                )}
              </div>
